...
namespace XV
{
  ...
  class CBitset
  {
    ...
    static constexpr ptrdiff_t type_bits_count = CHAR_BIT * sizeof(Type);
    static constexpr ptrdiff_t arr_size = Bits == 0 ? 0 : (Bits - 1) / type_bits_count;
  ...
  public:
    ...
    {
      const auto index_shift = static_cast<ptrdiff_t>(shift / type_bits_count);
      if (index_shift)
        for (ptrdiff_t i = arr_size; i >= 0; --i)
    ...
    constexpr CBitset &operator>>=(const u64 shift) noexcept
    {
      const auto index_shift = static_cast<ptrdiff_t>(shift / type_bits_count);
      if (index_shift)
        for (ptrdiff_t i = 0; i <= arr_size; ++i)
