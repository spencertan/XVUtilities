...
namespace XV
{
  ...
  struct Typelist
  {
    ...
    template <IsTypelist InputList>
    using Extend = decltype(XV::Types::Details::Concatenate<Types...>(InputList{}));

    template <template <typename> typename Predicate>
    using Find = decltype(XV::Types::Details::Find<Predicate>(Types{}...));

    template <template <typename> typename Predicate>
    using Remove = decltype(XV::Types::Details::Remove<Predicate>(Types{}...));

    template <size_t Dest>
    using Extract = decltype(XV::Types::Details::Extract<Dest, Types...>());

    template <IsNullType T = NullType>
    using Unique = decltype(XV::Types::Details::Unique(T{}, Types{}...));

    static constexpr size_t Size = sizeof...(Types);
    static constexpr bool Empty = Size == 0 ? true : false;

    template <typename CheckType>
    static constexpr int Index = XV::Types::Details::Index<0, CheckType, Types...>();
  };



  template <template <typename> typename Predicate, typename... Types>
  using TypesRemove = decltype(XV::Types::Details::Remove<Predicate>(Typelist<>{}, Typelist<Types...>{}));
  ...
  using ListFind = decltype(XV::Types::Details::Find<Predicate>(List{}));

  template <IsTypelist List, template <typename> typename Predicate>
  using ListRemove = decltype(XV::Types::Details::Remove<Predicate>(Typelist<>{}, List{}));
  template <IsTypelist List, size_t Dest>
  using ListExtract = List::template Extract<Dest>;

  template <IsTypelist List>
  using ListUnique = decltype(XV::Types::Details::Unique(List{}));
